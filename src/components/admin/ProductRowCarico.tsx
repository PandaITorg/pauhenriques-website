...
export interface CaricoRowData {
  id: string;
  name: string;
  brand: string;
...
interface Props {
  product: CaricoRowData;
  onUpdated: (next: CaricoRowData) => void;
  onDelete: () => void;
  deleting?: boolean;
}

export default function ProductRowCarico({
  product,
  onUpdated,
  onDelete,
  deleting,
  ...
  const [draft, setDraft] = useState<CaricoRowData>(product);
  ...
  const canSave = !nameInvalid && !saving;

  const persist = async (
    patch: Partial<CaricoRowData>,
    optimisticNext: CaricoRowData,
  ): Promise<boolean> => {
  ...
  const handleSave = async () => {
    if (!canSave) return;
    setSaving(true);
    const patch: Partial<CaricoRowData> = {};
    if (draft.name !== product.name) patch.name = draft.name.trim();
    if (draft.brand !== product.brand) patch.brand = draft.brand;
    if (draft.consultationWhatsapp !== product.consultationWhatsapp)
      patch.consultationWhatsapp = draft.consultationWhatsapp;
    ...
    const next: CaricoRowData = { ...product, ...patch };
  ...
  return (
    <tr className="border-b border-border-subtle hover:bg-surface-elevated transition-colors align-middle">
      ...
      <td className="p-4 text-text-main/60 max-w-xs">
        {mode === "edit" ? (
          <input
            type="text"
            placeholder="https://wa.me/593..."
            value={draft.consultationWhatsapp ?? ""}
            onChange={(e) =>
              setDraft({ ...draft, consultationWhatsapp: e.target.value })
            }
            className={`${inputBase} ${inputOk}`}
            disabled={saving}
          />
        ) : product.consultationWhatsapp ? (
          <span className="text-xs truncate block" title={product.consultationWhatsapp}>
            {product.consultationWhatsapp}
          </span>
        ) : (
