...
export async function sendPaymentFailed(
  ...
  try {
    const { error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: params.to,
      subject: `Pago no procesado - Orden ${params.orderId.slice(0, 8)}`,
      html: buildFailedHtml(params),
    });
    if (error) {
      console.error("[email] Failed to send payment failed:", error);
      return { success: false };
    }
    console.log("[email] Payment failed email sent to:", params.to);
    return { success: true };
  } catch (err) {
...
function buildFailedHtml(params: PaymentFailedParams): string {
  const itemsHtml = params.items
    .map(
      (item, index) => `
      <tr>
        <td style="padding: 14px 20px; ${index < params.items.length - 1 ? "border-bottom: 1px solid rgba(193,196,167,0.10);" : ""} color: #c1c4a7; font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 14px;">
          <span style="font-weight: 500;">${item.name}</span>
          <span style="color: #778a63; font-size: 13px; margin-left: 6px;">x${item.quantity}</span>
        </td>
        <td style="padding: 14px 20px; ${index < params.items.length - 1 ? "border-bottom: 1px solid rgba(193,196,167,0.10);" : ""} color: #c1c4a7; font-family: 'Helvetica Neue', Arial, sans-serif; font-size: 14px; text-align: right; white-space: nowrap;">
          $${(item.price * item.quantity).toFixed(2)}
        </td>
      </tr>`,
    )
...
<body style="margin: 0; padding: 0; background-color: #0e120a; font-family: 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #0e120a; padding: 40px 16px;">
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width: 560px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #2b3322; border: 1px solid rgba(193,196,167,0.15); border-radius: 16px; overflow: hidden;">
                ...
                <tr>
                  <td style="padding: 40px 36px 28px; text-align: center;">
                    <table cellpadding="0" cellspacing="0" role="presentation" style="margin: 0 auto 20px;">
                      <tr>
                        <td style="width: 56px; height: 56px; background-color: rgba(220,80,80,0.08); border: 2px solid rgba(220,80,80,0.20); border-radius: 50%; text-align: center; vertical-align: middle; line-height: 56px;">
                          <span style="color: #dc5050; font-size: 26px; font-weight: 300;">&#10005;</span>
                        </td>
                      </tr>
                    </table>
                    <h1 style="margin: 0 0 8px; font-family: Georgia, 'Times New Roman', serif; font-size: 22px; font-weight: 400; color: #c1c4a7; letter-spacing: 0.5px;">
                    ...
                    <p style="margin: 0; color: #778a63; font-size: 14px; line-height: 1.6;">
                      Hola, <strong style="color: #c1c4a7; font-weight: 500;">${params.customerName || "cliente"}</strong>. Tu pago no pudo completarse.
                    </p>
                  </td>
                </tr>
                  ...
                  <td style="padding: 24px 36px;">
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
                      <tr>
                        <td style="vertical-align: top; width: 50%;">
                          <span style="font-size: 11px; color: #556346; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600;">Orden</span>
                          <div style="color: #c1c4a7; font-size: 14px; font-family: Consolas, monospace; margin-top: 4px;">#${params.orderId.slice(0, 8)}</div>
                        </td>
                        <td style="vertical-align: top; width: 50%; text-align: right;">
                          <span style="font-size: 11px; color: #556346; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600;">Fecha</span>
                          <div style="color: #c1c4a7; font-size: 14px; margin-top: 4px;">${formattedDate}</div>
                ...
                <tr>
                  <td style="padding: 0 36px 24px;">
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: rgba(220,80,80,0.06); border: 1px solid rgba(220,80,80,0.15); border-radius: 10px;">
                      <tr>
                        <td style="padding: 14px 20px;">
                          <span style="font-size: 11px; color: #8a5050; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600;">Motivo</span>
                          <div style="color: #c1a0a0; font-size: 13px; margin-top: 4px; line-height: 1.5;">${params.errorMessage}</div>
                        </td>
                      </tr>
                    </table>
                  </td>
                ...
                <tr>
                  <td style="padding: 24px 36px 0;">
                    <span style="font-size: 11px; color: #556346; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600;">Productos en tu orden</span>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 8px 36px 32px;">
                    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #21281a; border: 1px solid rgba(193,196,167,0.10); border-radius: 10px;">
                      <tr>
                        <td style="padding: 16px 20px; font-size: 17px; font-weight: 600; color: #c1c4a7; font-family: Georgia, 'Times New Roman', serif;">Total</td>
                        <td style="padding: 16px 20px; font-size: 17px; font-weight: 700; color: #a68a63; text-align: right;">$${params.total.toFixed(2)}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
