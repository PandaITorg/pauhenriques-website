...
import equipoVentas from "@/assets/equipo-de-ventas.jpg";
...
export default function ProgramaAfiliadosClient() {
  ...
  useEffect(() => {
    if (country) {
      try {
        ...
        console.warn("Could not get calling code for", country.iso2);
        setPhone("");
      }
    } else {
      setPhone("");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (profilePicture && CLOUDINARY_UPLOAD_PRESET) {
